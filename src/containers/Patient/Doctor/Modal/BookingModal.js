...
import { savePatientBookAppointment } from "../../../../services/bookingService";
...
class BookingModal extends Component {
  ...
  handleConfirmBooking = async () => {
    ...
    let res = await savePatientBookAppointment({
    ...
    if (res && res.code === 200 && res.data) {
      ...
      this.setState({ isShowLoading: false });
      if (language === "en") {
        toast.warn("Your email is already in used, Please try another email!");
      } else {
        toast.warn("Email đã tồn tại, vui lòng chọn email khác!");
      }
    }
    else if (res && res.code === 405004) {
      this.setState({ isShowLoading: false });
      if (language === "en") {
        toast.warn("You have scheduled an appointment for this time period!");
      } else {
        toast.warn("Bạn đã đặt lịch khám trong khoảng thời gian này!");
      }
    }
    else {
      this.setState({ isShowLoading: false });
