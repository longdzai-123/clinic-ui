...
class Signup extends Component {
  ...
  createNewUser = async (data) => {
    try {
      ...
      if (response && response.code === 405001) {
        if (this.props.language == "en") {
          toast.error("Your email is already in used, Please try another email ! Sign up new account failed!");
        } else {
          toast.error("Email đã tồn tại, Vui lòng sử dụng emmail khác!");
        }
      }
